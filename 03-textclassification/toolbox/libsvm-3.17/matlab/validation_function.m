...
valid_function = @(dec, labels) fredoError(dec, labels);
...
function ret = accuracy(dec, label)
  ...
  function ret = fredoError(dec, label)
  correct = sum(dec .* label >= 0);
  fn =  sum(label == 1 & dec < 0);
  fp = sum(label == -1 & dec >= 0);
  ret = correct / (correct + fn + 5*fp);
  disp(sprintf('FredoError =  %g', ret));
